...
export function getMergedGuiaData() {
    ...
    // Clonar para no mutar el JSON base
    const mergedSections = rawData.sections.map(sec => {
        const secCustoms = customProcs.filter(cp => cp.sectionId === sec.id);
        const existingSubs = [...sec.subsections];

        secCustoms.forEach(cp => {
            // Si ya existe por subId, actualizar; si no, agregar
            const existingIdx = existingSubs.findIndex(s => s.id === cp.subId);
            const customSubObj = {
                id: cp.subId,
                title: '✨ ' + cp.title,
                content: `
                    <div style="background:rgba(59,130,246,0.08);border-left:4px solid var(--accent);padding:12px 16px;border-radius:4px;margin-bottom:16px;font-size:0.8rem;display:flex;justify-content:space-between;align-items:center;">
                        ...
                        <div style="display:flex;gap:6px;">
                            <button id="btn-edit-proc-${escapeHtml(cp.id)}" style="background:none;border:none;color:var(--accent);cursor:pointer;font-size:0.75rem;">✏️ Editar</button>
                            <button id="btn-delete-proc-${escapeHtml(cp.id)}" style="background:none;border:none;color:#ef4444;cursor:pointer;font-size:0.75rem;">🗑️ Eliminar</button>
                        </div>
                    </div>
                    ${cp.content}
                `,
            ...
            if (existingIdx !== -1) {
                existingSubs[existingIdx] = customSubObj;
            } else {
                existingSubs.push(customSubObj);
            }
        });
...
export function navigateTo(sectionId, subsectionId) {
    ...
    setTimeout(() => {
        ...
        // Vincular botones de editar/eliminar procedimiento si es personalizado
